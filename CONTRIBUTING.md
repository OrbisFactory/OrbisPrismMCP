...
2. **Ramas:** Crea una rama desde `develop` (por ejemplo `fix/descripcion` o `feat/nueva-funcionalidad`).
3. **Pull requests:** Envía un PR contra `develop`. Describe qué cambia y por qué. Si modificas comandos o mensajes al usuario, actualiza README y/o [CLI README](src/prism/entrypoints/cli/README.md) y las cadenas en `locales/es.json` y `locales/en.json`.
...
- **Comandos y ayuda:** Los comandos reales son `ctx`, `config`, etc. La ayuda (`help.py`) y los mensajes de error deben mostrar exactamente lo que el usuario debe escribir (por ejemplo `config set game_path`, no `config_impl set game_path`).

---

## Probar tus cambios

1. Instala en modo editable: `pip install -e .`
...
| `src/prism/entrypoints/cli/` | Comandos CLI (context/ctx, query, mcp, lang, config). |
